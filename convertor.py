...
def simple_image_concat(filename , frame_num, img_list, concat_axis):
	i_new = np.concatenate(tuple(img_list), axis=concat_axis)
	imageio.imwrite(filename + "/" + str(frame_num) + ".jpg", i_new[:, :, :])
...
def video_to_dataset(filename, concat_axis, concat_size, frame_range = [0,-1] , frame_clip=[0,0,-1,-1]
	...
	for i, im in enumerate(reader):
		...
		if i >= frame_range[0]:
			...
			if len(img_list) == concat_size:
				simple_image_concat(filename, i, img_list,concat_axis)
		if i != -1 and i >= frame_range[1]:
			break
